...
Разработанная реляционная модель данных для приложения изучения языков соответствует следующим нормальным формам:

* **Первая нормальная форма (1НФ):** Все таблицы в модели находятся в 1НФ, поскольку каждый атрибут содержит атомарные значения (неделимые), и нет повторяющихся групп атрибутов. 

* **Вторая нормальная форма (2НФ):** Модель соответствует 2НФ, так как она уже находится в 1НФ, и все неключевые атрибуты полностью зависят от всего первичного ключа. В таблицах с составными первичными ключами (например, `user_languages`, `collection_words`, `exercise_words`) неключевых атрибутов нет, они состоят только из внешних ключей, формирующих составной первичный ключ, что автоматически удовлетворяет условиям 2НФ. В остальных таблицах с простыми первичными ключами (например, `users`, `languages`, `collections`, ...) все неключевые атрибуты зависят от единственного первичного ключа.

* **Третья нормальная форма (3НФ):** Модель соответствует 3НФ, так как она уже находится в 2НФ, и в ней отсутствуют транзитивные зависимости неключевых атрибутов от первичного ключа (неключевые атрибуты не зависят от других неключевых атрибутов). Например, в таблице `collections` атрибуты `language_id` и `creator_id` являются внешними ключами и напрямую ссылаются на соответствующие первичные ключи других таблиц (`languages` и `users`), а не на какие-либо другие неключевые атрибуты в таблице `collections`. Аналогично, в таблице `attempts` `user_id` и `exercise_id` также являются внешними ключами, а `started_at`, `completed_at` и `score` напрямую зависят от `id` попытки, а не от других неключевых атрибутов внутри `attempts`.