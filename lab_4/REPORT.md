...
```sql
BEGIN;
SELECT nickname FROM users WHERE id = 1; -- получили 'user1'
-- (паузапока T2 делает и коммитит изменения)
SELECT nickname FROM users WHERE id = 1; -- получаем 'UpdatedUser', т.к. T2 закоммитилась
COMMIT;
```
